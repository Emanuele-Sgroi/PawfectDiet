...
import { ButtonLarge, Dropdown, InputWithIcon, InputWithImage } from "../index";
...
const DogProfileCreationStep2 = ({ onSubmit, name, profileData }) => {
  ...
  const workOptions = ["Athletic (e.g., Running, Agility)", "Service Dog"]; // Options ceated for the research porpuse only. It should contain more option in the future
  ...
  // // useEffect to update local state if profileData changes
  // useEffect(() => {
  //   setIsWorkingDog(profileData.isWorkingDog || "No");
  // }, [profileData.isWorkingDog]);
